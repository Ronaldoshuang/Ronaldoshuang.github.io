...
![SynchronizedDemo.class]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-2.png" | absolute_url }})
...
![对象，对象监视器，同步队列和线程状态的关系]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-3.png" | absolute_url }})
...
![synchronized的happens-before关系]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-4.png" | absolute_url }})
...
![线程A写共享变量]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-5.png" | absolute_url }})
...
![线程B读共享变量]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-6.png" | absolute_url }})
...
![Mark Word存储结构]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-7.png" | absolute_url }})
如图在Mark Word会默认存放hasdcode，年龄值以及锁标志位等信息。

Java SE 1.6中，锁一共有4种状态，级别从低到高依次是：**无锁状态、偏向锁状态、轻量级锁状态和重量级锁状态**，这几个状态会随着竞争情况逐渐升级。**锁可以升级但不能降级**，意味着偏向锁升级成轻量级锁后不能降级成偏向锁。这种锁升级却不能降级的策略，目的是为了提高获得锁和释放锁的效率。对象的MarkWord变化为下图：


![Mark Word状态变化]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-8.png" | absolute_url }})
...
![偏向锁撤销流程]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-9.png" | absolute_url }})
...
![偏向锁获取和撤销流程]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-10.png" | absolute_url }})
> **如何关闭偏向锁**
...
![轻量级锁加锁解锁以及锁膨胀]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-11.png" | absolute_url }})
因为自旋会消耗CPU，为了避免无用的自旋（比如获得锁的线程被阻塞住了），一旦锁升级成重量级锁，就不会再恢复到轻量级锁状态。当锁处于这个状态下，其他线程试图获取锁时，都会被阻塞住，当持有锁的线程释放锁之后会唤醒这些线程，被唤醒的线程就会进行新一轮的夺锁之争。
## 3.5 各种锁的比较 ##


![各种锁的对比]({{ "/images/posts/JavaConcurrency/JavaConcurrency-4-12.png" | absolute_url }})
# 4. 一个例子 #
